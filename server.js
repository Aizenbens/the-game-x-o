...
io.on('connection', (socket) => {
    ...
    socket.on('joinSnakeGame', (data) => {
        ...
        if (!allRooms[roomCode]) {
            allRooms[roomCode] = {
                ...
                snakes: {},
                candies: [],
                magnet: { x: 450, y: 350, taken: true, spawnTimer: 0 },
                interval: null
            };

            for (let j = 0; j < 50; j++) {
                allRooms[roomCode].candies.push({
                    x: Math.random() * 1250 + 35, y: Math.random() * 750 + 35,
                    type: candyIcons[Math.floor(Math.random() * candyIcons.length)]
                });
            }

            // توزيع سكنات عشوائية للبوتات الـ 14 في المودات المسموحة
            if (mode === 'global' || mode === 'bots') {
                for (let i = 1; i <= 14; i++) {
                    allRooms[roomCode].snakes["BOT_" + i] = {
                        ...
                        headSkin: ['default', 'crown', 'dragon', 'ninja'][Math.floor(Math.random() * 4)],
        ...
        // حفظ اسم السكن والبيانات للاعب الأونلاين
        allRooms[roomCode].snakes[socket.id] = {
            id: socket.id, name: name || "لاعب", color: color || "#10b981", headSkin: headSkin || "default",
    ...
    socket.on('disconnect', () => {
        ...
        if (rCode && allRooms[rCode]) {
            ...
            const realPlayers = Object.values(allRooms[rCode].snakes).filter(s => !s.isBot);
            if (realPlayers.length === 0) {
                clearInterval(allRooms[rCode].interval);
                delete allRooms[rCode];
            }
        }
    });
});
...
function runRoomEngine(roomCode) {
    ...
    room.interval = setInterval(() => {
        const keys = Object.keys(room.snakes);
        const deadPlayers = {};

        room.magnet.spawnTimer++;
        if (room.magnet.spawnTimer >= 100 && room.magnet.taken) {
            room.magnet.x = Math.random() * 1200 + 50;
            room.magnet.y = Math.random() * 700 + 50;
            room.magnet.taken = false;
            room.magnet.spawnTimer = 0;
        }

        keys.forEach(id => {
            const p = room.snakes[id];
            if (!p) return;
            ...
            if (!room.magnet.taken && Math.hypot(head.x - room.magnet.x, head.y - room.magnet.y) < 35) {
                room.magnet.taken = true;
                room.magnet.spawnTimer = 0;
                p.magnetActive = true;
                p.magnetTimeLeft = 50;
            }
        ...
        keys.forEach(id1 => {
            const p1 = room.snakes[id1]; if (!p1) return;
            const head1 = p1.body[0];

            keys.forEach(id2 => {
                if (id1 === id2) return;
                const p2 = room.snakes[id2]; if (!p2) return;
        ...
        for (let deadId in deadPlayers) {
            ...
            const deadPlayer = room.snakes[deadId];
            const killerPlayer = room.snakes[killerId];
            ...
            if (deadPlayer) {
                deadPlayer.body = [{ x: Math.random() * 1100 + 50, y: Math.random() * 700 + 50 }];
                deadPlayer.score = 0;
                deadPlayer.magnetActive = false;
            }
        }

        const leaderboard = Object.values(room.snakes)
            .map(p => ({ id: p.id, name: p.name, score: p.score }))
            .sort((a, b) => b.score - a.score);

        io.to(roomCode).emit('renderSnakeFrame', {
            snakes: Object.values(room.snakes),
...
server.listen(PORT, () => console.log(`3D Skins Room engine active on port ${PORT}`));
